...
image = modal.Image.debian_slim(python_version="3.10").pip_install(
    ...
    "aiohttp",          # Added aiohttp for asynchronous HTTP requests
    "faiss-cpu",        # For FAISS
...
def serve_vllm():
    ...
    async def get_completions(prompt: str, max_tokens: int = 2000, stream: bool = False):
...
def serve_fasthtml():
    ...
    if 'combined_text' not in df.columns:
        df['combined_text'] = df['Question']  # Use 'Question' column for embeddings (map similar user questions with Question col)
    ...
    async def ws(msg: str, send):
        ...
        system_prompt = (
            ...
            "and think through their choices, providing reassurance and objectivity. "
            "You have access to context information that may help answer the user's "
            "question. Use this context to provide accurate and helpful responses. "
        ...
        vllm_url = (
            f"https://{USERNAME}--{APP_NAME}-serve-vllm.modal.run/v1/completions"
        )
